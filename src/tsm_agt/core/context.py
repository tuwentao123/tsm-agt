...
class ContextWindowManager:
    ...
    def snapshot_data(self) -> dict[str, Any]:
        return {
            "algorithm": "structural-untrusted-summary",
            "version": 1,
    ...
    def _compact(
        ...
        summary = self._summary_message(compacted_messages)
        ...
        source_data = [self._message_hash_data(message) for message in compacted_messages]
        receipt = ContextCompactionReceipt(
            algorithm="structural-untrusted-summary", version=1,
            source_message_count=len(conversation),
            compacted_message_count=len(compacted_messages),
            preserved_message_count=len(preserved) - 1,
            ...
            summary_hash=canonical_hash(summary.to_data()["content"]),
            protected_message_count=sum(
                len(groups[index]) for index in protected_indexes
            ),
            protected_sections=(
                "current_user_input", "session_working_state",
